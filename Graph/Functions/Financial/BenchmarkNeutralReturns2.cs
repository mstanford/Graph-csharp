...
{
	public class BenchmarkNeutralReturns2 : Function
	{
		private readonly double _longLeverage;
		private readonly double _shortLeverage;

		public BenchmarkNeutralReturns2(double longLeverage, double shortLeverage)
		{
			_longLeverage = longLeverage;
			_shortLeverage = shortLeverage;
		}
		...
		public override double[] Evaluate2(double[] X1, double[] X2)
		{
			...
			for (int i = 1; i < Y.Length; i++)
			{
				...
				if (
					...
					!double.IsNaN(X2[i - 1]))
				{
					double longReturns = new Financial.LeveragedReturns().Update3(X1[i], X1[i - 1], _longLeverage) - 1.0;
					double shortReturns = new Financial.LeveragedReturns().Update3(X2[i], X2[i - 1], _shortLeverage) - 1.0;

					//if (longReturns != 0.0)
						Y[i] = Y[i - 1] * ((longReturns + shortReturns) + 1.0);
				}
			}
		...
		//            {
		//                double longReturns = new Financial.LeveragedReturns().Update3(longCumulativeReturnsColumn[i], longCumulativeReturnsColumn[i - 1], _longLeverage);
		//                double shortReturns = new Financial.LeveragedReturns().Update3(benchmarkCumulativeReturnsColumn[i], benchmarkCumulativeReturnsColumn[i - 1], _shortLeverage);
