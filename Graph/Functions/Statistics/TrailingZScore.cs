...
{
	public class TrailingZScore : Graph.Functions.Function
	{
		private readonly TrailingMean _mu;
		private readonly TrailingStandardDeviation _sigma;

		public TrailingZScore(double alpha)
		{
		...
		public override double Update(double x)
		{
			...
			_mu.Update(x);
			_sigma.Update(x);
			if (double.IsNaN(_mu.Value) || double.IsNaN(_sigma.Value))
				return double.NaN;
			Count++;
			Value = (x - _mu.Value) / _sigma.Value;
			return Value;
		}
