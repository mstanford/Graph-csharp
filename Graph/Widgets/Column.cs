...
namespace Graph.Widgets
{
	public class Column : Widget
	{
		...
		public Column(params Widget[] widgets)
			: base(widgets)
		{
		}
		...
		public override void Initialize(Scripting.Frame frame, Style style)
		{
			...
			for (int i = 0; i < Widgets.Length; i++)
			{
				width = System.Math.Max(width, Widgets[i].MinimumSize.Width);
				height += Widgets[i].MinimumSize.Height;
		...
		public override void Layout(System.Drawing.RectangleF rectangle)
		{
			...
			for (int i = 0; i < Widgets.Length; i++)
			{
				float height = Widgets[i].MinimumSize.Height + (heightIncrement * Widgets[i].ExpandableHeightCount);
				Widgets[i].Layout(new System.Drawing.RectangleF(x, y, rectangle.Width, height));
				y += height;
			}

			RenderedRectangle = new System.Drawing.RectangleF(rectangle.X, rectangle.Y, rectangle.Width, y - rectangle.Y);
		}
		...
		public override void Render(System.Drawing.Graphics graphics, System.Drawing.RectangleF rectangle)
		{
			...
			{
				float height = Widgets[i].MinimumSize.Height + (heightIncrement * Widgets[i].ExpandableHeightCount);
				Widgets[i].Render(graphics, new System.Drawing.RectangleF(x, y, rectangle.Width, height));
				y += height;
			}
